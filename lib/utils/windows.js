...
const getPowershell = () => {
    return path.join(process.env.windir, ((process.env.PROCESSOR_ARCHITECTURE === 'AMD64') ? 'System32/WindowsPowerShell/v1.0/powershell.exe' : 'SysWOW64/WindowsPowerShell/v1.0/powershell.exe'));
}
...
module.exports = {
    ...
    getStatus() {
        return new Promise((resolve, reject) => {
            Promise.all([this.exec('netsh wlan show hostednetwork'), this.getNetworkAdaptors(), this.getInternetConnectedAdaptor(), this.getCompatible()])
    ...
    getNetworkAdaptors() {
        return new Promise((resolve, reject) => {
            this.exec(util.format('"%s" "Get-NetAdapter | ft Name, Status, ifIndex, MacAddress, InterfaceDescription"', getPowershell()))
                .then(output => {
    ...
    getInternetConnectedAdaptor() {
        return new Promise((resolve, reject) => {
            this.exec(util.format('"%s" "Get-NetConnectionProfile"', getPowershell()))
                .then(output => {
                    var networkData = output.split('\n')
                        .map(Function.prototype.call, String.prototype.trim)
                        .filter(Boolean);
    ...
    getLocalIp(AdapterID) {
        return new Promise(resolve => {
            this.exec(util.format('"%s" "(Get-NetAdapter -ifIndex "%s" | Get-NetIPAddress).IPv4Address"', getPowershell(), AdapterID))
                .then(output => {
