...
module.exports = {
    getPowershell() {
        return path.join(process.env.windir, ((arch === 'x64') ? 'SysWOW64/WindowsPowerShell/v1.0/powershell.exe' : 'System32/WindowsPowerShell/v1.0/powershell.exe'));
    },
    getArch() {
    ...
    getStatus() {
        return new Promise((resolve, reject) => {
            this.getArch()
                .then(Promise.all.bind(this, [this.exec('netsh wlan show hostednetwork'), this.getNetworkAdaptors(), this.getInternetConnectedAdaptor(), this.getCompatible()]))
